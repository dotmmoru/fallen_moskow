...
(async function () {  
  ...
  const [ship,rocket, fire_1,fire_2,fire_3,fire_4, explosion, move_hand, face] = await Promise.all([
      Scene.root.findFirst('Ship'),
      Scene.root.findFirst('rocket'),
      ...
      Scene.root.findFirst('move_hand_hint'),
      FaceTracking.face(0)
  ]);
  ...
  let durationrocketMS = 2000;
  const timeDriverParametersRocket = 
  {
      durationMilliseconds: durationrocketMS,
      loopCount: 1,
      mirror: false
  };
...
function CheckRocketPosision()
{
  ...
  face.mouth.openness.monitor().subscribe(function (value) 
    {   
        if( value.oldValue > 0.3 && isAttacked === false)
        {
            PlayRocket();
            isAttacked = true;
        }     
...
function PlayRocket() 
{
    let timeDriver = Animation.timeDriver(timeDriverParametersRocket);
    let linearSampler = Animation.samplers.linear(-1000,280);
    let translationAnimation = Animation.animate(timeDriver, linearSampler);
    // Move object 
    rocket.transform.y = translationAnimation;
...
Diagnostics.watch("Openness ", face.mouth.openness);
